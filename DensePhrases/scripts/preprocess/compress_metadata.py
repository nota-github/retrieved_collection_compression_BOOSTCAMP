...
def compress(d):
    for i in d:
        ...
        d[i]["word2char_start"] = blosc.compress(
            word2char_start, typesize=1, cname="zlib"
        )
        ...
        try:
            decompressed_word2char_start = np.frombuffer(
                blosc.decompress(d[i]["word2char_start"]), type1
            )
            decompressed_word2char_end = np.frombuffer(
                blosc.decompress(d[i]["word2char_end"]), type2
            )
            decompressed_f2o_start = np.frombuffer(
                blosc.decompress(d[i]["f2o_start"]), type3
            )
...
def load_doc_groups(phrase_dump_dir):
    phrase_dump_paths = sorted(
        [
            ...
            for name in os.listdir(phrase_dump_dir)
            if "hdf5" in name
        ]
    )
    doc_groups = {}
...
def main(args):
    ...
    print(
        f"compressed by {round(size_after_compression/size_before_compression*100,2)}%"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dump_dir",
        type=str,
        default="dump/sbcd_sqdqgnqqg_inb64_s384_sqdnq_pinb2_0_20181220_concat/dump/phrase",
    )
