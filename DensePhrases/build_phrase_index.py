...
def concat_vectors(vectors):
    ...
    if len(vectors[0].shape) > 1:
        out_vector = np.zeros(
            (total_size, *vectors[0].shape[1:]), dtype=vectors[0].dtype
        )
    else:
...
def sample_data(
    dump_paths, doc_sample_ratio=0.2, vec_sample_ratio=0.2, seed=29, norm_th=999
):
    start_vecs = []
    end_vecs = []
    random.seed(seed)
    ...
    for i, f in enumerate(tqdm(dumps)):
        ...
        for doc_id in tqdm(sampled_doc_ids, desc="sampling from %d" % i):
            ...
            for group in groups:
                ...
                if num_start == 0:
                    continue
                sampled_start_idxs = np.random.choice(
                    num_start, int(vec_sample_ratio * num_start)
                )
                start_vec = int8_to_float(
                    start_set, group.attrs["offset"], group.attrs["scale"]
                )[sampled_start_idxs]
...
def train_index(
    ...
    # Default index type
        ...
        print("OPQ indexing")
        if hnsw:
            start_index = faiss.IndexHNSWPQ(
                ds, "HNSW32,PQ96", faiss.METRIC_INNER_PRODUCT
            )
        else:
            print("define transform with OPQMatrix")
            opq_matrix = faiss.OPQMatrix(ds, code_size)
            opq_matrix.niter = 10
            sub_index = faiss.IndexIVFPQ(
                quantizer, ds, num_clusters, code_size, 8, faiss.METRIC_INNER_PRODUCT
            )
    ...
    if cuda:
        ...
        co.useFloat16 = True
        print("move faiss index from cpu to gpu. this may takes long time")
        ...
        # import pdb
        # pdb.set_trace()
        # TODO. please figure out why gpu_index.train() requires 256 centroids
...
def add_with_offset(
    start_index, start_data, start_valids, start_total, offset, fine_quant
):
    if "none" in fine_quant:
        start_index.add(start_data)
    else:
        start_ids = (np.arange(start_data.shape[0]) + offset + start_total).astype(
            np.int64
        )
    ...
    for di, phrase_dump in enumerate(tqdm(dumps, desc="dumps")):
        ...
        for i, (doc_idx, doc_group) in enumerate(
            ...
            # Apply index filter
                ...
                o2f_start = {
                    orig: ft for ft, orig in enumerate(doc_group["f2o_start"][:])
                }
            ...
            if len(starts) > 0 and (
                (i % num_docs_per_add == 0) or (i == dump_length - 1)
            ):
...
def merge_indexes(
    ...
    idx2id_paths = [
        os.path.join(subindex_dir, name) for name in names if name.endswith(".hdf5")
    ]
    index_paths = [
        os.path.join(subindex_dir, name) for name in names if name.endswith(".faiss")
    ]
    ...
    invlists = faiss.OnDiskInvertedLists(index.nlist, index.code_size, target_inv_path)
...
def run_index(args):
    ...
    if args.stage == "move":
        ...
        invlists = faiss.OnDiskInvertedLists(
            index.nlist, index.code_size, args.inv_path
        )
        index.replace_invlists(invlists)
