...
class DensePhrases(object):
    def __init__(self,
        ...
        # Load MIPS        
        if os.path.exists(self.args.phrase_index_serialized_path):
            print(f"load mips phrase indices from {self.args.phrase_index_serialized_path}")
            # v2. joblib
            self.mips = joblib.load(self.args.phrase_index_serialized_path)

            # v1. pickle
            #with open(self.args.phrase_index_serialized_path, 'rb') as fr: # pickle
                #self.mips = pickle.load(fr) # pickle
                
        else:
            print("construct mips phrase indices based on dump file ...")
            self.mips = load_phrase_index(self.args, ignore_logging=not verbose)
            print(f"saving {self.args.phrase_index_serialized_path}")
            
            try:
                # v2. joblib
                joblib.dump(self.mips, self.args.phrase_index_serialized_path)
                # v1. pickle
                #with open(self.args.phrase_index_serialized_path, 'wb') as fw:
                #    pickle.dump(self.mips, fw)

            except Exception as e:
                print(f'dump failed with message: "{str(e)}"')
                os.remove(self.args.phrase_index_serialized_path)
