...
class Encoder(PreTrainedModel):
    ...
    def __init__(
        ...
        # Encoders: three LMs
        self.phrase_encoder = (
            pretrained if pretrained is not None else transformer_cls(config)
        )
    ...
    def merge_inputs(self, input_ids_, attention_mask_, input_ids, attention_mask):
        ...
        for input_id_, att_mask_, input_id, att_mask in zip(
            ...
            new_input_id[
                : att_mask_.sum() + att_mask.sum() - 1 - title_sep
            ] = torch.cat([input_id_[: att_mask_.sum()], sep_input_id], dim=0)
                ...
                (
                    torch.cat(
                        [
                            ...
                            torch.zeros(
                                max_len - att_mask.sum(0) - att_mask_.sum(0) + 1
                            )
                            .long()
                            .to(self.device),
                        ]
    ...
    def forward(
        ...
        if dist.is_initialized() and self.training:
            ...
            qs_list = [
                torch.zeros_like(query_start) for _ in range(dist.get_world_size())
            ]
            qe_list = [
                torch.zeros_like(query_end) for _ in range(dist.get_world_size())
            ]
            sp_list = [
                torch.zeros_like(start_positions) for _ in range(dist.get_world_size())
            ]
            ep_list = [
                torch.zeros_like(end_positions) for _ in range(dist.get_world_size())
            ]
            ...
            if neg_input_ids is not None:
                nps_list = [
                    torch.zeros_like(neg_start) for _ in range(dist.get_world_size())
                ]
                npe_list = [
                    torch.zeros_like(neg_end) for _ in range(dist.get_world_size())
                ]
        ...
        if neg_input_ids is not None:
            neg_start_logits = (
                ...
                .sum(-1)
                .view(
                    all_query_start.shape[0],
                    all_neg_start.shape[0],
                    all_neg_start.shape[1],
                )
                .max(-1)[0]
            )
            neg_end_logits = (
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if self.lambda_kl > 0:
                ...
                with torch.no_grad():
                    ...
                    for seq_output, att_mask_, input_id in zip(
                        ...
                        title_mask = torch.zeros(
                            size=(title_sep.item(), seq_output.shape[1])
                        ).to(self.device)
                ...
                kl_start = (
                    kl_loss(
                        ...
                        target=softmax(
                            start_logits_qd[:, : start_logits.size(1)], dim=1
                        ),
                    ).sum(1)
                ).mean(0)
                kl_end = (
            ...
            if self.lambda_flt > 0:
                length = torch.tensor(filter_start_logits.size(-1)).to(
                    filter_start_logits.device
                )
    ...
    def train_query(
        ...
        if start_vecs is not None:
            if all([len(t) == 0 for t in targets]) and all(
                [len(t) == 0 for t in p_targets]
            ):
                return None, None
        ...
        if not all([len(t) == 0 for t in targets]):
            log_probs = [
                -torch.log(softmax(lg, -1)[tg.long()].sum().clamp(MIN_PROB, 1))
            ...
            start_loss = [
                -torch.log(softmax(lg, -1)[tg.long()].sum().clamp(MIN_PROB, 1))
                for lg, tg in zip(start_logits, targets)
                if len(tg) > 0
            ]
            end_loss = [
                -torch.log(softmax(lg, -1)[tg.long()].sum().clamp(MIN_PROB, 1))
                for lg, tg in zip(end_logits, targets)
                if len(tg) > 0
            ]
            log_probs = (
                log_probs
                + sum(start_loss) / len(start_loss)
                + sum(end_loss) / len(end_loss)
            )
            import pdb
        ...
        if not all([len(t) == 0 for t in p_targets]):
            ...
            p_start_loss = [
                -torch.log(softmax(lg, -1)[tg.long()].sum().clamp(MIN_PROB, 1))
                for lg, tg in zip(p_start_logits, p_targets)
                if len(tg) > 0
            ]
            ...
            p_end_loss = [
                -torch.log(softmax(lg, -1)[tg.long()].sum().clamp(MIN_PROB, 1))
                for lg, tg in zip(p_end_logits, p_targets)
                if len(tg) > 0
            ]
            log_probs = (
                log_probs
