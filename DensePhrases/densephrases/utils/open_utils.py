...
def load_phrase_index(args, ignore_logging=False):
    ...
    mips = MIPS(
        ...
        if ignore_logging
        else (logging.DEBUG if args.verbose_logging else logging.INFO),
    )
    return mips
...
def load_cross_encoder(device, args):
    ...
    ce_model = torch.nn.ModuleList([model, qa_outputs])
    ce_model.to(device)
...
def load_qa_pairs(data_path, args, q_idx=None, draft_num_examples=100, shuffle=False):
    ...
    for data_idx, item in enumerate(data):
        ...
        if "[START_ENT]" in question:
            question = question[
                max(question.index("[START_ENT]") - 300, 0) : question.index(
                    "[END_ENT]"
                )
                + 300
            ]
...
def load_ptkb_qa_pairs(
    data_path, args, q_idx=None, draft_num_examples=100, shuffle=False
):
    q_ids = []
    questions = []
    answers = []
    titles = []
    ptkb = json.load(open(data_path))
    for idx, data in enumerate(ptkb):
        for data_idx, item in enumerate(data["turns"]):
            if q_idx is not None:
                if data_idx != q_idx:
                    continue
            q_id = item["turn_id"]
            if "origin" in item:
                q_id = item["number"] + "-" + q_id
            question = item["resolved_utterance"]
            if "[START_ENT]" in question:
                question = question[
                    max(question.index("[START_ENT]") - 300, 0) : question.index(
                        "[END_ENT]"
