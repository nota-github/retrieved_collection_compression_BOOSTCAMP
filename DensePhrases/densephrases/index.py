...
class MIPS(object):
    def __init__(
        ...
        if "none" not in index_path:  # not FlatIndex
            self.reconst_fn = faiss.downcast_index(self.index.index).reconstruct
            self.R = torch.FloatTensor(
                faiss.vector_to_array(
                    faiss.downcast_VectorTransform(self.index.chain.at(0)).A
                ).reshape(self.index.d, self.index.d)
            )
        else:
            self.reconst_fn, self.R = None, None
    ...
    def open_dumps(self):
        if os.path.isdir(self.phrase_dump_dir):
            self.phrase_dump_paths = sorted(
                [
                    os.path.join(self.phrase_dump_dir, name)
                    for name in os.listdir(self.phrase_dump_dir)
                    if "hdf5" in name
                ]
            )
            dump_names = [
            ...
            if "-" in dump_names[0] and ("dev" not in dump_names[0]):  # Range check
                ...
                self.dump_ranges = [(0, 1e8)] * len(
                    dump_names
                )  # TODO: fix herustics. allow larger number of range
        else:
    ...
    def decompress_meta(self, doc_idx):
        ...
        f2o_start = np.frombuffer(
            blosc.decompress(self.doc_groups[doc_idx]["f2o_start"]), dtypes["f2o_start"]
        )
    ...
    def get_idxs(self, I):
        ...
        doc = np.array(
            [
                [self.idx_f[str(offset)]["doc"][idx] for offset, idx in zip(oo, ii)]
                for oo, ii in zip(offsets, idxs)
            ]
        )
        word = np.array(
            [
                [self.idx_f[str(offset)]["word"][idx] for offset, idx in zip(oo, ii)]
                for oo, ii in zip(offsets, idxs)
            ]
        )
        return doc, word
    ...
    def search_phrase(
        ...
        query = np.reshape(
            np.tile(np.expand_dims(query, 1), [1, top_k, 1]), [-1, query.shape[1]]
        )
        q_idxs = np.reshape(
            np.tile(np.expand_dims(np.arange(num_queries), 1), [1, top_k * 2]), [-1]
        )
        ...
        assert (
            len(start_doc_idxs) == len(start_idxs) == len(end_idxs) == len(start_scores)
        )
        ...
        default_doc = [
            doc_idx
            for doc_idx in set(start_doc_idxs.tolist() + end_doc_idxs.tolist())
            if doc_idx >= 0
        ][0]
            ...
            groups_start = [
                {
                    "end": np.array(
                        [
                            groups[doc_idx]["start"][ii]
                            for ii in range(
                                start_idx,
                                min(
                                    start_idx + max_answer_length,
                                    len(groups[doc_idx]["start"]),
                                ),
                            )
                        ]
                    )
        ...
        # Get metadata from RAM
            ...
            for doc_idx, start_idx in zip(start_doc_idxs, orig_start_idxs):
                ...
                for ii in range(start_idx, start_idx + max_answer_length):
                    try:
                        reconst = self.reconst_fn(ii)  # not work for FlatIndex
                    except:
                        reconst = np.zeros(bs)
            ...
            for doc_idx, end_idx in zip(end_doc_idxs, orig_end_idxs):
                ...
                for ii in range(end_idx - max_answer_length + 1, end_idx + 1):
                    try:
                        reconst = self.reconst_fn(ii)  # not work for FlatInex
                    except:
                        reconst = np.zeros(bs)
                    reconsts.append(reconst)
        ...
        def valid_phrase(start_idx, end_idx, doc_idx, max_ans_len):
            ...
            if (
                groups_all[doc_idx]["f2o_start"][end_idx]
                - groups_all[doc_idx]["f2o_start"][start_idx]
                > max_ans_len
            ):
                return False
        ...
        new_end_idxs = [
            [
                start_idx + i
                if valid_phrase(start_idx, start_idx + i, doc_idx, max_answer_length)
                else -1
        ...
        for end_idx, each_end in enumerate(ends):
                ...
                float(groups_all[default_doc]["offset"]),
                float(groups_all[default_doc]["scale"]),
                each_end,
            )

        with torch.no_grad():
            end = torch.FloatTensor(end).to(self.device)
            if self.R is not None:  # FlatIndex
        ...
        pred_end_idxs = np.stack(
            [each[idx] for each, idx in zip(new_end_idxs, np.argmax(scores1, 1))], 0
        )  # [Q]
        pred_end_vecs = np.stack(
            [each[idx] for each, idx in zip(end.cpu().numpy(), np.argmax(scores1, 1))],
            0,
        )
        ...
        start_time = time()
        starts = [
            group_end["start"] for end_idx, group_end in zip(end_idxs, groups_end)
        ]
        new_start_idxs = [
            [
                end_idx - i
                if valid_phrase(end_idx - i, end_idx, doc_idx, max_answer_length)
                else -1
                for i in range(max_answer_length - 1, -1, -1)
            ]
        ...
        with torch.no_grad():
            ...
            if self.R is not None:  # not flatIndex
        ...
        scores2 = (
            new_start_scores + np.expand_dims(end_scores, 1) + start_mask
        )  # [Q, L]
        pred_start_idxs = np.stack(
            [each[idx] for each, idx in zip(new_start_idxs, np.argmax(scores2, 1))], 0
        )  # [Q]
        pred_start_vecs = np.stack(
            [
                each[idx]
                for each, idx in zip(start.cpu().numpy(), np.argmax(scores2, 1))
            ],
            0,
        )
        ...
        doc_idxs = np.concatenate(
            (np.expand_dims(start_doc_idxs, 1), np.expand_dims(end_doc_idxs, 1)), axis=1
        ).flatten()
        start_idxs = np.concatenate(
            (np.expand_dims(start_idxs, 1), np.expand_dims(pred_start_idxs, 1)), axis=1
        ).flatten()
        end_idxs = np.concatenate(
            (np.expand_dims(pred_end_idxs, 1), np.expand_dims(end_idxs, 1)), axis=1
        ).flatten()
        max_scores = np.concatenate(
            (np.max(scores1, 1, keepdims=True), np.max(scores2, 1, keepdims=True)),
            axis=1,
        ).flatten()
        ...
        if return_idxs:
            ...
            start_vecs = np.concatenate(
                (
                    np.expand_dims(
                        np.stack(
                            [group_start["end"][0] for group_start in groups_start]
                        ),
                        1,
                    ),
                    np.expand_dims(pred_start_vecs, 1),
                ),
                axis=1,
            ...
            end_vecs = np.concatenate(
                (
                    ...
                    np.expand_dims(
                        np.stack([group_end["start"][-1] for group_end in groups_end]),
                        1,
                    ),
                ),
                axis=1,
        ...
        out = [
            {
                "context": groups_all[doc_idx]["context"],
                "title": [groups_all[doc_idx]["title"]],
                "doc_idx": doc_idx,
                "start_pos": groups_all[doc_idx]["word2char_start"][
                    groups_all[doc_idx]["f2o_start"][start_idx]
                ].item(),
                "end_pos": (
                    groups_all[doc_idx]["word2char_end"][
                        groups_all[doc_idx]["f2o_start"][end_idx]
                    ].item()
                    if (len(groups_all[doc_idx]["word2char_end"]) > 0)
                    and (end_idx >= 0)
            ...
            for group_idx, (doc_idx, start_idx, end_idx, score) in enumerate(
                zip(
                    doc_idxs.tolist(),
                    start_idxs.tolist(),
                    end_idxs.tolist(),
        ...
        for i in range(len(new_out)):
            ...
            new_out[i] = list(
                filter(lambda x: x["score"] > -1e5, new_out[i])
    ...
    def search(
        ...
        if aggregate:
            outs = [
                self.aggregate_results(results, top_k, q_text, agg_strat)
                for results, q_text in zip(outs, q_texts)
            ]
