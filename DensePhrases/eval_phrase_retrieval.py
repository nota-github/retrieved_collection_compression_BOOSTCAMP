...
from densephrases.utils.open_utils import (
    load_phrase_index,
    get_query2vec,
    load_qa_pairs,
    load_ptkb_qa_pairs,
...
def evaluate(args, mips=None, query_encoder=None, tokenizer=None, q_idx=None):
    ...
    if os.path.basename(os.path.dirname(args.test_path)) == "ptkb":
        qids, questions, answers, _ = load_ptkb_qa_pairs(args.test_path, args, q_idx)
    else:
    ...
    for q_idx in tqdm(range(0, len(questions), step)):
        result = mips.search(
            ...
            q_texts=questions[q_idx : q_idx + step],
            nprobe=args.nprobe,
            top_k=args.top_k,
        ...
        prediction = [
            [ret["answer"] for ret in out][: args.top_k] if len(out) > 0 else [""]
            for out in result
        ]
        evidence = [
            [ret["context"] for ret in out][: args.top_k] if len(out) > 0 else [""]
            for out in result
        ]
        title = [
            [ret["title"] for ret in out][: args.top_k] if len(out) > 0 else [[""]]
            for out in result
        ]
        score = [
            [ret["score"] for ret in out][: args.top_k] if len(out) > 0 else [-1e10]
            for out in result
        ]
        se_pos = [
            [(ret["start_pos"], ret["end_pos"]) for ret in out][: args.top_k]
            if len(out) > 0
            else [(0, 0)]
            for out in result
        ]
...
def evaluate_results(
    ...
    if args.candidate_path is not None:
        ...
        topk_preds = [
            list(filter(lambda x: (x in candidates) or (x.lower() in candidates), a))
    ...
    for i in range(len(predictions)):
        ...
        em_topk = max(
            [
                drqa_metric_max_over_ground_truths(match_fn, prediction, answers[i])
...
def evaluate_results_kilt(
    ...
    pred_wikipedia_ids = [
        [[title2wikiid[t] for t in title_] for title_ in title] for title in titles
    ]
    ...
    pred_official_path = os.path.join(
        pred_dir,
        f'{args.load_dir.split("/")[-1]}_'
        + os.path.splitext(os.path.basename(args.test_path))[0]
        + f"_{total}.jsonl",
    )
    official_preds_to_save = []
    for prediction, title, question, pred_wikipedia_id, qid in zip(
        predictions, titles, questions, pred_wikipedia_ids, qids
    ):
        if (
            "wned" in pred_official_path
            or "cweb" in pred_official_path
            or "aidayago2" in pred_official_path
        ):
            answer = title[0][0]
        else:
        ...
        output = {
            "answer": answer,
            "provenance": [
                {"wikipedia_id": pred_wid_}
                for pred_wid in pred_wikipedia_id
                for pred_wid_ in pred_wid
            ],
        }
        official_preds_to_save.append(
            {"id": qid, "input": question, "output": [output]}
        )
    ...
    result = kilt_evaluate(gold=args.kilt_gold_path, guess=pred_official_path)
    ...
    pred_path = os.path.join(
        pred_dir,
        os.path.splitext(os.path.basename(args.test_path))[0] + f"_{total}.pred",
    )
    ...
    return (
        result["retrieval"]["Rprec"],
        result["retrieval"]["recall@5"],
        result["kilt"]["KILT-accuracy"],
        result["kilt"]["KILT-f1"],
    )
...
def evaluate_results_psg(pred_path, args):
    ...
    for qid, pred in tqdm(enumerate(my_pred.values())):
        ...
        # truncate
        pred = {
            key: val[: args.psg_top_k]
            if key in ["evidence", "title", "se_pos", "prediction"]
            else val
        ...
        assert (
            len(set(pred["evidence"])) == len(pred["evidence"]) == len(pred["title"])
        ), "Should use opt2 for aggregation"
        ...
        assert all(
            len(ctx["text"].split()) <= args.max_psg_len for ctx in my_dict["ctxs"]
        )
    ...
    out_file = os.path.join(
        os.environ["SAVE_DIR"],
        os.path.basename(args.load_dir),
        "pred",
