        ...
        if isinstance(single_query_or_queries_dict, dict): # batch search
            ...
            with open(f"{RUNFILE_DIR}/{self.args.runfile_name}", "w") as fw:
                ...
                for batch_query in tqdm(queries_batch):
                    ...
                    if self.args.static:
                        result_phrase, meta_phrase= self.model.search(batch_query, retrieval_unit='phrase', top_k=TOP_K)
                        ...
                        for sentences, phrase_answer_list in zip(result, result_phrase):
                            ...
                            phrase_sentence.append(phrase_answer_list_no_subset + sentences) 
            ...
            result, meta = self.model.search(single_query_or_queries_dict, retrieval_unit=R_UNIT, top_k=TOP_K)
            
            breakpoint()
            if self.args.static:
                phrase_sentence = []
                result_phrase, meta_phrase= self.model.search(single_query_or_queries_dict, retrieval_unit='phrase', top_k=TOP_K)
                phrase_answer_list_no_subset =[]
